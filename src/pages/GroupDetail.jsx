...
export function Live({ memberList, live, totals, weekly, currentUid, onSelectMember, onInvite }) { const liveMembers = memberList.filter((m) => { const status = live[m.uid]?.status; return status === 'active' || status === 'paused' || status === 'onBreak' }); const idleMembers = memberList.filter((m) => !liveMembers.some((x) => x.uid === m.uid)); const ranked = [...memberList].map((m) => ({ ...m, seconds: weekly?.[m.uid] || 0 })).sort((a, b) => b.seconds - a.seconds); const tierFor = (uid) => { const status = weekly?.[uid] > 0 ? leagueStatus(ranked, uid) : null; return status && status.name !== 'Unranked' ? status : null }; return <div className="animate-fade-in">
  ...
  <div className="grid grid-cols-3 gap-3 mb-8">
    ...
    {liveMembers.map((m) => { const status = live[m.uid]?.status; const paused = status === 'paused' || status === 'onBreak'; return <LiveTile key={m.uid} member={m} seconds={totals[m.uid] || 0} self={m.uid === currentUid} paused={paused} status={status} tier={tierFor(m.uid)} onClick={() => onSelectMember(m.uid)} /> })}
  </div>
...
function TierBadge({ tier }) { if (!tier) return null; return <span className={`absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full bg-bg border-2 border-bg flex items-center justify-center ${tier.textClass}`}><LeagueIcon width={13} height={13} /></span> }
...
function LiveTile({ member, seconds, self, paused, status, tier, onClick }) { const label = paused ? (status === 'onBreak' ? 'BREAK' : 'PAUSED') : 'STUDYING'; return <button onClick={onClick} className={`flex flex-col items-center gap-2 bg-surface border border-border rounded-2xl py-4 ${paused ? 'opacity-55' : ''}`}><div className="relative"><Avatar name={member.displayName} photoURL={member.photoURL} size="md" live={!paused} showDot={false} /><TierBadge tier={tier} /></div><span className="text-xs font-medium truncate max-w-full px-1">{self ? 'You' : member.displayName}</span><span className={`text-[10px] tracking-wide ${paused ? 'text-text-faint' : 'text-live'}`}>{label}</span><span className={`text-xs tabular-nums ${paused ? 'text-text-faint' : 'text-live'}`}>{formatDuration(seconds)}</span></button> }
function IdleTile({ member, seconds, self, tier, onClick }) { return <button onClick={onClick} className="flex flex-col items-center gap-2 bg-surface border border-border rounded-2xl py-4 opacity-70"><div className="relative"><Avatar name={member.displayName} photoURL={member.photoURL} size="md" /><TierBadge tier={tier} /></div><span className="text-xs font-medium text-text-dim truncate max-w-full px-1">{self ? 'You' : member.displayName}</span><span className="text-xs tabular-nums text-text-faint">{formatDuration(seconds)}</span></button> }
