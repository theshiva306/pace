...
export default function GroupDetail() {
  ...
  return <div className={`px-5 pt-[calc(env(safe-area-inset-top)+20px)] pb-6 max-w-md mx-auto md:max-w-2xl md:pt-14 flex flex-col ${tab === 'Chat' ? 'h-[var(--pace-viewport-height,100dvh)] min-h-0 overflow-hidden' : 'min-h-svh'}`}> 
    ...
    {tab === 'Live' && <Live memberList={memberList} live={live || {}} totals={daily || {}} currentUid={user.uid} onSelectMember={setMemberSheetUid} onInvite={() => setInviteOpen(true)} />}
...
function RefreshRow({ label }) { return <div className="mb-4"><div className="text-[13px] tracking-[0.25em] text-text-faint">{label}</div></div> }
...
function getLeaderboardRankDisplay(seconds, index) {
  if (seconds <= 0) return '—'
  if (index < 3) return <LeagueIcon width="18" height="18" className={RANK_COLOR[index]} />
  return index + 1
}
function Leaderboard({ memberList, totals, currentUid, week, onOpenWeekPicker, onSelectMember }) { const ranked = [...memberList].map((m) => ({ ...m, seconds: totals[m.uid] || 0 })).sort((a, b) => b.seconds - a.seconds); const currentSeconds = totals[currentUid] || 0; const mine = currentSeconds > 0 ? leagueStatus(ranked, currentUid, { final: !week.isCurrent }) : null; return <div className="animate-fade-in"><div className="flex items-center justify-between mb-4 gap-3"><button onClick={onOpenWeekPicker} className="flex items-center gap-1.5 text-left"><div><div className="text-sm font-semibold tracking-tight">{week.label}</div><div className="text-[11px] text-text-faint">{week.isCurrent ? `Ends in ${week.daysLeft} day${week.daysLeft === 1 ? '' : 's'}` : 'Session ended'}</div></div><ChevronDown className="text-text-faint mt-2.5" /></button>{mine && <div className="flex items-center gap-2 shrink-0"><LeagueIcon className={mine.textClass} /><div className="text-right"><div className={`text-xs font-semibold tracking-wide ${mine.textClass}`}>{mine.name} league</div><div className="text-[11px] text-text-faint">{mine.detail}</div></div></div>}</div><RefreshRow label="STANDINGS" /><div className="flex flex-col">{ranked.map((m, i) => <button key={m.uid} onClick={() => onSelectMember(m.uid)} className={`flex items-center gap-4 py-3 border-b border-border-soft last:border-0 text-left ${m.uid === currentUid ? 'bg-accent-soft/40 -mx-3 px-3 rounded-xl' : ''}`}><span className="w-7 flex items-center justify-center text-sm text-text-faint tabular-nums">{getLeaderboardRankDisplay(m.seconds, i)}</span><Avatar name={m.displayName} photoURL={m.photoURL} size="sm" /><span className="flex-1 text-sm font-medium truncate">{m.displayName}</span><span className="text-sm tabular-nums text-text-dim">{formatDuration(m.seconds)}</span></button>)}</div></div> }
export function Live({ memberList, live, totals, currentUid, onSelectMember, onInvite }) { const liveMembers = memberList.filter((m) => { const status = live[m.uid]?.status; return status === 'active' || status === 'paused' || status === 'onBreak' }); const idleMembers = memberList.filter((m) => !liveMembers.some((x) => x.uid === m.uid)); return <div className="animate-fade-in">
  <SectionDivider dotClassName="bg-live" label={`${liveMembers.length} focusing`} labelClassName="text-live" />
  <div className="grid grid-cols-3 gap-x-3 gap-y-5 mb-8">
    <InviteTile onClick={onInvite} />
    {liveMembers.map((m) => { const status = live[m.uid]?.status; const paused = status === 'paused' || status === 'onBreak'; return <LiveTile key={m.uid} member={m} seconds={totals[m.uid] || 0} self={m.uid === currentUid} paused={paused} status={status} onClick={() => onSelectMember(m.uid)} /> })}
  </div>
  {idleMembers.length > 0 && <>
    <SectionDivider dotClassName="bg-text-faint/60" label="Not focusing" labelClassName="text-text-dim" />
    <div className="grid grid-cols-3 gap-x-3 gap-y-5">
      {idleMembers.map((m) => <IdleTile key={m.uid} member={m} seconds={totals[m.uid] || 0} self={m.uid === currentUid} onClick={() => onSelectMember(m.uid)} />)}
    </div>
  </>}
</div> }
...
function InviteTile({ onClick }) { return <button onClick={onClick} className="flex flex-col items-center gap-2 text-center"><div className="w-14 h-14 rounded-full border border-dashed border-border-soft flex items-center justify-center text-text-faint"><PlusIcon /></div><span className="text-xs text-text-faint leading-tight px-1">Invite a friend</span></button> }
function LiveTile({ member, seconds, self, paused, status, onClick }) { const label = paused ? (status === 'onBreak' ? 'BREAK' : 'PAUSED') : 'STUDYING'; return <button onClick={onClick} className="flex flex-col items-center gap-2 text-center"><Avatar name={member.displayName} photoURL={member.photoURL} size="md" /><span className="text-xs font-medium truncate max-w-full px-1">{self ? 'You' : member.displayName}</span><span className={`text-[10px] tracking-wide ${paused ? 'text-text-faint' : 'text-live'}`}>{label}</span><span className={`text-xs tabular-nums ${paused ? 'text-text-faint' : 'text-live'}`}>{formatDuration(seconds)}</span></button> }
function IdleTile({ member, seconds, self, onClick }) { return <button onClick={onClick} className="flex flex-col items-center gap-2 text-center"><Avatar name={member.displayName} photoURL={member.photoURL} size="md" /><span className="text-xs font-medium text-text-dim truncate max-w-full px-1">{self ? 'You' : member.displayName}</span><span className="text-xs tabular-nums text-text-faint">{formatDuration(seconds)}</span></button> }
