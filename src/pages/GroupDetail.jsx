...
import { ChevronLeft, ChevronDown, CopyIcon, ShareIcon, SendIcon, SettingsIcon, RefreshIcon, ExitIcon, TrashIcon, InviteIcon, LeagueIcon } from '../components/icons'
...
export default function GroupDetail() {
  ...
  return <div className="min-h-svh px-5 pt-[calc(env(safe-area-inset-top)+20px)] pb-32 max-w-md mx-auto md:max-w-2xl md:pt-14 flex flex-col">
    ...
    {tab === 'Leaderboard' && <Leaderboard memberList={memberList} totals={weekly.value || {}} currentUid={user.uid} onRefresh={weekly.refresh} refreshing={weekly.refreshing} updatedAt={weekly.updatedAt} week={week} onOpenWeekPicker={() => setWeekPickerOpen(true)} onSelectMember={setMemberSheetUid} />}
    {tab === 'Live' && <Live memberList={memberList} live={live.value || {}} totals={daily.value || {}} currentUid={user.uid} onRefresh={daily.refresh} refreshing={daily.refreshing} updatedAt={daily.updatedAt} onSelectMember={setMemberSheetUid} />}
...
function RefreshRow({ label, onRefresh, refreshing, updatedAt }) { return <div className="flex items-center justify-between mb-4"><div className="text-[13px] tracking-[0.25em] text-text-faint">{label}</div><button onClick={onRefresh} disabled={refreshing} className="flex items-center gap-1.5 text-xs text-text-faint hover:text-text-dim disabled:opacity-50"><RefreshIcon className={refreshing ? 'animate-spin' : ''} />{refreshing ? 'Refreshing…' : updatedAt ? 'Refresh' : 'Loading…'}</button></div> }
