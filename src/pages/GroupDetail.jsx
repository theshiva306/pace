...
function TierBadge({ tier, size = 'md' }) { if (!tier) return null; const box = size === 'sm' ? 'w-[17px] h-[17px] rounded-[5px] -bottom-1 -right-1' : 'w-[22px] h-[22px] rounded-[7px] -bottom-1.5 -right-1.5'; const icon = size === 'sm' ? 11 : 14; return <span className={`absolute flex items-center justify-center shadow-md shadow-black/40 ${box} ${tier.chipClass}`}><LeagueIcon width={icon} height={icon} className="text-bg" /></span> }
